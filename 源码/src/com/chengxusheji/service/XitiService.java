...
public class XitiService {
    ...
    /*按照查询条件分页查询习题记录*/
    public ArrayList<Xiti> queryXiti(InfoClass infoClassObj,String title,String addTime,int currentPage) throws Exception { 
     	String where = "where 1=1";
    	if(null != infoClassObj && infoClassObj.getInfoClassId()!= null && infoClassObj.getInfoClassId()!= 0)  where += " and t_xiti.infoClassObj=" + infoClassObj.getInfoClassId();
    	if(!title.equals("")) where = where + " and t_xiti.title like '%" + title + "%'";
    	if(!addTime.equals("")) where = where + " and t_xiti.addTime like '%" + addTime + "%'";
    	int startIndex = (currentPage-1) * this.rows;
    	return xitiMapper.queryXiti(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<Xiti> queryXiti(InfoClass infoClassObj,String title,String addTime) throws Exception  { 
     	String where = "where 1=1";
    	if(null != infoClassObj && infoClassObj.getInfoClassId()!= null && infoClassObj.getInfoClassId()!= 0)  where += " and t_xiti.infoClassObj=" + infoClassObj.getInfoClassId();
    	if(!title.equals("")) where = where + " and t_xiti.title like '%" + title + "%'";
    	if(!addTime.equals("")) where = where + " and t_xiti.addTime like '%" + addTime + "%'";
