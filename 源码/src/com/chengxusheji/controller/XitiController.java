...
public class XitiController extends BaseController {
	...
	public void list(@ModelAttribute("infoClassObj") InfoClass infoClassObj,String title,String addTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Xiti> xitiList = xitiService.queryXiti(infoClassObj, title, addTime, page);
	    /*计算总的页数和总的记录数*/
	    xitiService.queryTotalPageAndRecordNumber(infoClassObj, title, addTime);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("infoClassObj") InfoClass infoClassObj,String title,String addTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (title == null) title = "";
		if (addTime == null) addTime = "";
		List<Xiti> xitiList = xitiService.queryXiti(infoClassObj, title, addTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    xitiService.queryTotalPageAndRecordNumber(infoClassObj, title, addTime);
	    /*获取到总的页码数目*/
	    int totalPage = xitiService.getTotalPage();
	...
	public void OutToExcel(@ModelAttribute("infoClassObj") InfoClass infoClassObj,String title,String addTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(title == null) title = "";
        if(addTime == null) addTime = "";
        List<Xiti> xitiList = xitiService.queryXiti(infoClassObj,title,addTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Xiti信息记录"; 
        String[] headers = { "习题id","知识分类","习题标题","分值","发布时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<xitiList.size();i++) {
        	Xiti xiti = xitiList.get(i); 
        	dataset.add(new String[]{xiti.getXitiId() + "",xiti.getInfoClassObj().getInfoClassName(),xiti.getTitle(),xiti.getScore() + "",xiti.getAddTime()});
        }
        /*
